...
class FnService:
    ...
    async def handle(self, *args, **kwargs):
        ...
        future = asyncio.get_event_loop().run_in_executor(
            None, lambda: self.algorithm_config.fn(*args, **kwargs)
        )
        out = await asyncio.wait_for(
            future, timeout=self.algorithm_config.service_timeout
        )
        return out
    ...
    def launch_thread(
        ...
        while True:
            try:
                ...
                param_result, param_err_msg = self.param_validator.validate(
                    *args, **kwargs
                )
                if not param_result:
...
def get_available_fn_queue(
    ...
    fn_req_queue_list: List[multiprocessing.Queue],
    fn_res_queue_list: List[multiprocessing.Queue],
) -> Tuple[Lock, multiprocessing.Queue, multiprocessing.Queue]:
    """获取当前可用的函数锁、函数队列"""
...
class ApiService:
    ...
    def __init__(
        self, algorithm_config: ConfigLoader, algorithm_info: Algoinfo
    ) -> None:
        self.algorithm_config = algorithm_config
    ...
    def read_file(self, dict_or_path: Union[dict, str]) -> str:
        ...
        if type(dict_or_path) == dict:
            ...
            file_name = (
                create_timestamp_filename() + "." + file_dict.get("format", "")
                if file_dict.get("format")
                else create_timestamp_filename()
            )
            ...
            if file_dict.get("type", "base64") == "base64":
                ...
                raise Exception(
                    f'Invalid file type: {file_dict.get("type")}, only support base64 and url.'
                )
        else:
    ...
    def write_file(self, local_path: str) -> str:
        ...
        response = requests.post(
            upload_url, files=files, headers=login_instance.get_header()
        )
        if (
            response.status_code != 200
            and response.status_code != 201
            and response.json()["status"] != 200
        ):
    ...
    def get_example(self):
        ...
        example += (
            ...
            + ", ".join(
                [
                    f"{key}: {input_dict['type']}"
                    for key, input_dict in self.algorithm_config.service_input.items()
                ]
            )
            + ")"
        )
...
class GradioService:
    ...
    def launch(
        ...
        def fn(*args):
            ...
            for index, (key, _) in enumerate(
                self.algorithm_config.service_input.items()
            ):
        ...
        for _, info in self.algorithm_config.service_input.items():
            inputs.append(
                self.get_input_component(
                    info["type"], info["describe"], info.get("default", None)
                )
            )
        outputs = []
        ...
        self.check_port()
        threading.Thread(
            target=self.check_launched, args=(gradio_port_con,), daemon=True
        ).start()
    ...
    def check_port(self):
        ...
        while True:
            ...
            state = sock.connect_ex(
                (
                    self.algorithm_config.gradio_server_host,
    ...
    async def handle(self, input_info: dict) -> dict:
        ...
        if req_method == "POST" and req_headers["Content-Type"] == "application/json":
            res = await asyncio.get_event_loop().run_in_executor(
                None, lambda: requests.post(url, json=req_body, headers=req_headers)
            )
        if req_method == "POST" and req_headers["Content-Type"] != "application/json":
            res = await asyncio.get_event_loop().run_in_executor(
                None, lambda: requests.post(url, data=req_body, headers=req_headers)
            )
...
class Service:
    ...
    def __init__(
        ...
        self.algorithm_info = Algoinfo(
            self.algorithm_config.name, self.algorithm_config.version
        )
    ...
    def launch(
        ...
        try:
            ...
            if type == "FN":
                ...
                self.launch_service(
                    fn_lock_list, fn_req_queue_list, fn_res_queue_list, gradio_port_con
                )
            elif type == "GRADIO":
                if not gradio_port_con:
                    raise Exception("Argument 'gradio_port_con' is None.")
            ...
            def on_req(req_info: dict, loop: asyncio.AbstractEventLoop):
                ...
                try:
                    ...
                    future = asyncio.run_coroutine_threadsafe(
                        self.handle(
                            req_info, fn_lock_list, fn_req_queue_list, fn_res_queue_list
                        ),
                        loop,
                    )
                    if not self.is_exclude_parallel(req_info):
    ...
    async def ask_data(self, id: Optional[str] = None) -> Optional[dict]:
        ask_for_data_resp = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: requests.get(
                self.ask_data_url,
                params={
        ...
        if (
            ask_for_data_resp.status_code != 200
            and ask_for_data_resp.status_code != 201
        ):
            raise Exception(
                ask_for_data_resp.status_code, ask_for_data_resp.content.decode()
            )
    ...
    def is_exclude_parallel(self, req_info: dict):
        """是否排除计算并行数外"""
        return req_info["type"] == "gradio" and self.gradio_service.is_nouse_fn(
            req_info["data"]
        )
            ...
            try:
                ...
                if req_info["type"] == "api":
                    ...
                    result = await asyncio.get_event_loop().run_in_executor(
                        None,
                        lambda: self.api_service.handle(
                            req_info["data"],
                            fn_lock_list,
                            fn_req_queue_list,
                            fn_res_queue_list,
                ...
                return_ans_resp = await asyncio.get_event_loop().run_in_executor(
                    None,
                ...
                if (
                    return_ans_resp.status_code != 200
                    and return_ans_resp.status_code != 201
                    and return_ans_resp.json()["status"] != 200
                ):
    ...
    def login(self):
        if not login(
            secret=self.algorithm_config.secret,
            username=self.algorithm_config.username,
            password=self.algorithm_config.password,
            privilege="PROVIDER",
        ):
            self.algo_logger.error(
                f"Login failed. Please check your secret or password."
            )
            return False
        return True
    ...
    async def start(self):
        ...
        fn_lock_list = [
            ...
            for _ in range(self.algorithm_config.service_max_parallel)
        ]  # 模型函数的锁列表
        fn_req_queue_list = [
        ...
        def create_fn_process():
            fn_process = multiprocessing.Process(
                target=self.launch,
                args=(
                    stdio_queue,
                    "FN",
        ...
        async def join_task(
            process_creator: Callable[[], Tuple[multiprocessing.Process, str]]
        ):
        ...
        async def subprocess_stdio_task():
            ...
            while True:
                try:
                    ...
                    is_execed = await asyncio.get_event_loop().run_in_executor(
                        None, stdio_exec
                    )
                except concurrent.futures._base.CancelledError:
                    stdio_exec_all()
