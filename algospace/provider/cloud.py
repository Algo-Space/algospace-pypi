...
def run_cloud_deploy(config_path: str, reset: bool = False):
    ...
    if not login(
        secret=algorithm_config.secret,
        username=algorithm_config.username,
        password=algorithm_config.password,
        privilege="PROVIDER",
    ):
    ...
    if (
        reset
        and get_service_status(algorithm_config.name, algorithm_config.version)
        == "built"
    ):
        try:
    ...
    if get_service_status(algorithm_config.name, algorithm_config.version) == "unready":
        ...
        try:
            start_build_image(
                algorithm_config.name, algorithm_config.version, config_path
            )
    ...
    if get_service_status(algorithm_config.name, algorithm_config.version) in (
        "built",
        "building",
        "pending",
        "unready",
        ...
        while get_service_status(algorithm_config.name, algorithm_config.version) in (
            "building",
            "pending",
        ):
            time.sleep(1)
        if (
            get_service_status(algorithm_config.name, algorithm_config.version)
            == "unready"
        ):
            algospace_logger.error("Build failed.")
            algospace_logger.error(
                "Please check your code and config file according to the build log."
            )
            algospace_logger.error(
                "After modification, please run `algospace cloud:deploy` again."
            )
            algospace_logger.error(
    ...
    if get_service_status(algorithm_config.name, algorithm_config.version) in (
        ...
        try:
            algospace_logger.info(
                "It may take a few minutes to distribute the image to calculation node."
            )
            algospace_logger.info(
                "And then the deploy log will be shown below. Please wait patiently."
            )
        ...
        if (
            ...
            algospace_logger.error(
                "Please check your code and config file according to the deploy log."
            )
            algospace_logger.error(
                "After modification, please run `algospace cloud:deploy --reset` to deploy again."
            )
            algospace_logger.error(
                f"Or go to {algorithm_info.algorithm_cloud_site} to replace part of files uploaded. (Avoid re-uploading the entire code)"
            )
        ...
        if (
            ...
            algospace_logger.info(
                "You can run `algospace cloud:log` to view the running log."
            )
...
def show_running_log(config_path: str):
    ...
    if (
        ...
        algospace_logger.warning(
            "The service is not deployed. Please run `algospace cloud:deploy` first."
        )
    ...
    try:
        ...
        with open(zip_name, "rb") as f:
            ...
            response = requests.post(
                f"{upload_code_url}/{name}/{version}",
                data=m,
...
def get_deploy_log(
    name: str, version: str, keep_after_success: bool = False, last_log_row_num: int = 0
):
    """获取部署日志"""
