...
class InputType(ParamType):
    ...
    def Float(
        ...
        min_value: Optional[int] = None,
        max_value: Optional[int] = None,
        max_fraction: Optional[int] = None,
...
valid_input_type = [
    getattr(InputType, x) for x in dir(InputType) if not x.startswith("__")
]
valid_output_type = [
    getattr(OutputType, x) for x in dir(OutputType) if not x.startswith("__")
]
...
class ConfigLoader:
    def __init__(
        self,
        config_path: str,
        is_verify_service: bool = False,
        is_verify_self_gradio_launch: bool = False,
    ) -> None:
        ...
        self.service_input_examples: list[dict] = getattr(
            config, "service_input_examples", []
        )
        ...
        self.gradio_launch_host: str = getattr(
            config, "gradio_launch_host", "127.0.0.1"
        )
        self.gradio_launch_port: int = getattr(config, "gradio_launch_port", 7860)
        self.is_self_gradio_launch = bool(
            self.gradio_launch_filepath and self.gradio_launch_function
        )
    ...
    def _verify(self) -> None:
        ...
        if not bool(re.match("^[a-zA-Z0-9_\-@]+$", self.name)):
            raise ConfigError(
                "ConfigError: 'name' can only include 'a-z A-Z 0-9 _ - @'."
            )
        ...
        if not bool(re.match("^[a-zA-Z0-9\.]+$", self.version)):
            raise ConfigError(
                "ConfigError: 'version' can only include 'a-z A-Z 0-9 .'."
            )
        ...
        if len(self.secret) == 0 and (
            len(self.password) == 0 or len(self.username) == 0
        ):
        ...
        for key, info in self.service_input.items():
            if type(key) != str:
                raise ConfigError(
                    f"ConfigError: key '{str(key)}' in 'service_input' is not str."
                )
            if type(info) != dict:
                raise ConfigError(
                    f"ConfigError: value of '{str(key)}' in 'service_input' is not dict."
                )
            ...
            if (
                info.get("max_length", None) is not None
                and info["type"] != InputType.STRING
            ):
                raise ConfigError(
                    f"ConfigError: max_length of '{str(key)}' in 'service_input' is not allowed."
                )
            if (
                info.get("max_length", None) is not None
                and type(info["max_length"]) != int
            ):
                raise ConfigError(
                    f"ConfigError: max_length of '{str(key)}' in 'service_input' is not int."
                )
            if info.get("min_value", None) is not None and info["type"] not in [
                InputType.INTEGER,
                InputType.FLOAT,
            ]:
                raise ConfigError(
                    f"ConfigError: min_value of '{str(key)}' in 'service_input' is not allowed."
                )
            if info.get("min_value", None) is not None and type(
                info["min_value"]
            ) not in [int, float]:
                raise ConfigError(
                    f"ConfigError: min_value of '{str(key)}' in 'service_input' is not int or float."
                )
            if info.get("max_value", None) is not None and info["type"] not in [
                InputType.INTEGER,
                InputType.FLOAT,
            ]:
                raise ConfigError(
                    f"ConfigError: max_value of '{str(key)}' in 'service_input' is not allowed."
                )
            if info.get("max_value", None) is not None and type(
                info["max_value"]
            ) not in [int, float]:
                raise ConfigError(
                    f"ConfigError: max_value of '{str(key)}' in 'service_input' is not int or float."
                )
            if (
                info.get("min_value", None) is not None
                and info.get("max_value", None) is not None
                and info["min_value"] > info["max_value"]
            ):
                raise ConfigError(
                    f"ConfigError: min_value of '{str(key)}' in 'service_input' is greater than max_value."
                )
            if (
                info.get("max_fraction", None) is not None
                and info["type"] != InputType.FLOAT
            ):
                raise ConfigError(
                    f"ConfigError: max_fraction of '{str(key)}' in 'service_input' is not allowed."
                )
            if (
                info.get("max_fraction", None) is not None
                and type(info["max_fraction"]) != int
            ):
                raise ConfigError(
                    f"ConfigError: max_fraction of '{str(key)}' in 'service_input' is not int."
                )
            if info.get("max_size", None) is not None and info["type"] not in [
                InputType.IMAGE_PATH,
                InputType.VIDEO_PATH,
                InputType.VOICE_PATH,
            ]:
                raise ConfigError(
                    f"ConfigError: max_size of '{str(key)}' in 'service_input' is not allowed."
                )
            if info.get("max_size", None) is not None and type(
                info["max_size"]
            ) not in [int, float]:
                raise ConfigError(
                    f"ConfigError: max_size of '{str(key)}' in 'service_input' is not int or float."
                )
            if (
                info.get("default", None) is not None
                and info["type"]
                in [
                    InputType.STRING,
                    InputType.IMAGE_PATH,
                ...
                and type(info["default"]) != str
            ):
                raise ConfigError(
                    f"ConfigError: default of '{str(key)}' in 'service_input' is not str."
                )
            if (
                info.get("default", None) is not None
                and info["type"] in [InputType.INTEGER, InputType.FLOAT]
                and type(info["default"]) not in [int, float]
            ):
                raise ConfigError(
                    f"ConfigError: default of '{str(key)}' in 'service_input' is not int or float."
                )
            if info.get("default", None) is not None and info["type"] in [
                InputType.IMAGE_PATH,
                InputType.VIDEO_PATH,
                InputType.VOICE_PATH,
            ]:
                ...
                if not os.path.exists(info["default"]):
                    raise ConfigError(
                        f"ConfigError: default of '{str(key)}' in 'service_input' does not exist."
                    )
        ...
        for key, info in self.service_output.items():
            if type(key) != str:
                raise ConfigError(
                    f"ConfigError: key '{str(key)}' in 'service_output' is not str."
                )
            if type(info) != dict:
                raise ConfigError(
                    f"ConfigError: value of '{str(key)}' in 'service_output' is not dict."
                )
        ...
        for index, input_example in enumerate(self.service_input_examples):
            if type(input_example) != dict:
                raise ConfigError(
                    "ConfigError: 'service_input_examples' contains not dict."
                )
            ...
            for key, value in input_example.items():
                ...
                if self.service_input[key]["type"] in [
                    InputType.IMAGE_PATH,
                    InputType.VIDEO_PATH,
                    InputType.VOICE_PATH,
                ]:
        ...
        if self.scope not in ["PRIVATE", "GROUP", "INSTITUTION", "PUBLIC"]:
            raise ConfigError(
                "ConfigError: 'scope' is not in 'PRIVATE','GROUP','INSTITUTION','PUBLIC'."
            )
        ...
        if self.document_filepath and not self._read_document_file():
            raise ConfigError(
                f"ConfigError: '{self.document_filepath}' does not exist."
            )
        ...
        if self.is_self_gradio_launch:
            ...
            if not os.path.exists(
                self._get_gradio_launch_file_info()["gradio_launch_filepath"]
            ):
                raise ConfigError(
                    f"ConfigError: file '{self.gradio_launch_filepath}' does not exist."
                )
    ...
    def verify_service(self) -> None:
        if self._import_service_file() is None:
            raise ConfigError(f"ConfigError: '{self.service_function}' does not exist.")
    ...
    def _get_service_file_info(self) -> dict:
        service_path = os.path.join(
            self.config_dirpath, os.path.split(self.service_filepath)[0]
        )
    ...
    def _get_gradio_launch_fn(self) -> Optional[Callable]:
        return getattr(
            self._import_gradio_launch_file(), self.gradio_launch_function, None
        )

    def _import_gradio_launch_file(self):
        gradio_launch_path = self._get_gradio_launch_file_info()["gradio_launch_path"]
        gradio_launch_filename_noext = self._get_gradio_launch_file_info()[
            "gradio_launch_filename_noext"
        ]
    ...
    def _get_gradio_launch_file_info(self) -> dict:
        gradio_launch_path = os.path.join(
            self.config_dirpath, os.path.split(self.gradio_launch_filepath)[0]
        )
        ...
        gradio_launch_filepath = os.path.join(
            gradio_launch_path, gradio_launch_filename
        )
        return {
