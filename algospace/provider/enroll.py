    ...
    try:
        ...
        if not login(
            ...
            algospace_logger.error(
                "Login failed. Please check your secret or password."
            )
            exit(-1)
...
def enroll(
    ...
    enroll_data = {
        ...
        "input_param": [
            {
                ...
                **(
                    ...
                    if val.get("max_length") is not None
                    else {}
                ),
                **(
                ...
                **(
                    {"max_fraction": val.get("max_fraction")}
                    if val.get("max_fraction") is not None
                    else {}
                ),
                **(
                    {"max_size": val.get("max_size")}
                    if val.get("max_size") is not None
                    else {}
                ),
            }
        ...
        "output_param": [
            {"key": key, "type": val["type"], "describe": val["describe"]}
