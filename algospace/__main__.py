...
def run():
    ...
    subparsers = parser.add_subparsers(
        dest=ArgNamespace.command
    )  # ArgNamespace.command

    init_parser = subparsers.add_parser(
        "init", description="初始化算法服务配置", help="初始化算法服务配置"
    )
    start_parser = subparsers.add_parser("start", description="启动算法服务", help="启动算法服务")
    generate_parser = subparsers.add_parser(
        "generate", description="生成容器配置", help="生成容器配置"
    )
    enroll_parser = subparsers.add_parser("enroll", description="注册算法服务", help="注册算法服务")
    cloud_deploy_parser = subparsers.add_parser(
        "cloud:deploy", description="部署云端托管算法", help="部署云端托管算法"
    )
    cloud_log_parser = subparsers.add_parser(
        "cloud:log", description="查看云端托管算法日志", help="查看云端托管算法日志"
    )

    # 主命令参数
    parser.add_argument(
        "-v", "--version", dest=ArgNamespace.version, action="store_true", help="显示版本信息"
    )
    # start 命令参数
