...
from algospace.provider.config_loader import (
    InputType,
    OutputType,
    valid_input_type,
    valid_output_type,
...
class AlgoFunction:
    ...
    def convert_file_dict(self, local_path: str) -> dict:
        ...
        if local_path.startswith("http"):
            format = re.sub(
                r"\?.*$", "", os.path.splitext(local_path)[-1].replace(".", "")
            )
            return {
                "type": "url",
                "data": local_path,
    ...
    def __call__(self, *args: list, **kwds: dict) -> dict:
        ...
        if not can_call:
            raise Exception(
                "This algorithm is not callable now because the provider closed the service."
            )
        ...
        for param in input_param:
            if param["key"] not in kwargs.keys():
                self.show_example()
                raise Exception(
                    "Arguments are not correct. Please refer to the example above."
                )
            param_key = param["key"]
        ...
        if response.json()["status"] != 200:
            raise CallException(
                response.json().get("err_msg", "Call algorithm function error.")
            )
        ...
        for param in output_param:
            param_key = param["key"]
            param_type = param["type"]
            result[param_key] = self.get_output_type_class(param_type)(
                result[param_key]
            )
        return result
    ...
    def get_info(self) -> dict:
        ...
        if not self.info:
            ...
            if len(algo_list) == 0:
                raise Exception(
                    f'No algorithm named {self.name + (" " + self.version if self.version else "")}.'
                )
            algo = algo_list[0]
            self.info = {
                "name": algo["name"],
